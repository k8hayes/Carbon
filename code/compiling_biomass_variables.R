# adding density, BA and SOL into biomass file to model
...
# Set up ################
biomass <- read.csv(here("data/output/biomass_plot.csv")) # no CWD

  biomass$Biomass_gm2 <- biomass$Overstory + biomass$Understory

  colnames(biomass)
  biomass <- biomass %>%
    select(!c(TAB, CWD, Understory, Overstory))
...
test <- merge(biomass, density, by = c("PLOT", "SITE", "TREAT"))
...
# Soil organic layer #####################################
soil <- read.csv(here("data/soil/org_depth.csv"))
...
## Merging ###################
data <- merge(data, soil, by = c("SITE", "TREAT", "PLOT"))

write.csv(data, here("data/output/biomass_variables.csv"), row.names = F)
