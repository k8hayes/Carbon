...
lm1_jags <- function(){
  # Likelihood:
  for (i in 1:N){
    sheight[i] ~ dnorm(z[i], tau2) # tau is precision (1 / variance)
    z[i] ~ dnorm(mu[i], tau)
...
init_values <- function(){ # specifies initial parameter values
  list(alpha = rnorm(1), beta = rnorm(1), sigma = runif(1), sigma2 = runif(1))
}

params <- c("alpha", "beta", "sigma", "sigma2") # chooses the parameters for which to report posteriors
