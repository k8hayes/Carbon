...
int_plot <- ggplot(fit_int, aes(Value, Coef)) +
  ...
  scale_y_discrete(expand = expansion(add = c(0.2, 2))) +
  geom_vline(xintercept = 0, linetype = "dashed", col = "darkgrey") +
  theme_bw() + xlab("Posterior estimate") + ylab("") + ggtitle("Biomass Posterior Distributions",
...
jagsdata_0 <- with(data, # creates a list of elements
                   list(y = biomass_y,
                        N = length(data$Biomass_gm2)))
...
mod0_jags <- function(){
  # Likelihood:
  for (i in 1:N){
    y[i] ~ dlnorm(mu, tau) # tau is precision (1 / variance)
  }
  mu ~ dnorm(0, 1.0E-06)
  tau <- 1/(sigma * sigma)
  sigma ~ dgamma(0.01, 0.01)
...
# initial values ############################
init_values0 <- function(){ # specifies initial parameter values
  list(mu = rnorm(1))
}
...
# Run the model in JAGS ###########################################
fit_mod0 <- jags(data = jagsdata_0, # specifies data
                inits = init_values0,
                parameters.to.save = params0,
                model.file = mod0_jags,
                n.chains = 3, # number of chains
                n.iter = 1000,
                n.burnin = 0) # only keeps every 10th iteration)
fit_mod0
...
# Calculate r2 value ################################
total.var <- mean(fit_mod0$BUGSoutput$sims.list$tau)

unexplained.var <- mean(fit_mod$BUGSoutput$sims.list$tau)
