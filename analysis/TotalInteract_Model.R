...
# START #####################################
soil <- read.csv(here("data/output/soil_variables.csv"))
biomass <- read.csv(here("data/output/biomass_variables.csv"))

data <- merge(soil, biomass, by = c("SITE", "TREAT", "PLOT",
                                    "avSOLdepth_CM", "dens_gm2",
                                    "decidba_gm2"))

data$total_gm2 <- data$Soil_gm2 + data$Biomass_gm2
...
total_y <- as.numeric(data$total_gm2)
nPlot <- 41
...
log.glm <- glm(log(total_gm2) ~ data$TREAT, family = gaussian, data = data)
